...
class MY_Log
{
     ...
     * @param array $arr            
     */
    private function _log($log_level_name, $arr)
    {
        // 日志已关闭
        if ($this->_enabled === FALSE) {
            return FALSE;
        ...
        // 初始化本条日志串
        $str = sprintf("[log_id=%s][log_level=%s][log_time=%s:%-06d][%s]", $this->_log_id, $log_level_name, date("Y-m-d H:i:s", $sec), $ms, $file_line_str);
